...
interface WeddingMusicManagerProps {
  progress: number; // Pass progress directly for more accurate detection
}

// Sadi (Wedding) section boundaries
// Sadi is at position 0.68 (68% of world width)
// Start slightly before (0.63) and end slightly after (0.73) for smooth transitions
const WEDDING_START = 0.63;
const WEDDING_END = 0.73;

const WeddingMusicManager: React.FC<WeddingMusicManagerProps> = ({ 
  progress
}) => {
  ...
  // Calculate if we're in wedding section based on progress
  const isInWeddingSection = progress >= WEDDING_START && progress <= WEDDING_END;

  useEffect(() => {
    // Ensure we're in the browser
    if (typeof window === 'undefined') return;
    
    // Create audio element for mangalam.mp3
      ...
      setTimeout(() => {
        if (audioEl.error) {
          ...
          if (hasRealError || (networkError && !audioLoadedRef.current)) {
            console.error('Failed to load mangalam.mp3:', {
              code: errorCode,
  ...
  useEffect(() => {
    ...
    if (!isInWeddingSection && audioRef.current) {
      audioRef.current.pause();
      audioRef.current.currentTime = 0;
    ...
    if (isInWeddingSection === previousInSectionRef.current) return;
    
    previousInSectionRef.current = isInWeddingSection;

    if (isInWeddingSection) {
      // Entering wedding section - stop any other audio elements first
      ...
      // Also stop any audio from SoundManager or SangeetMusicManager by finding their refs
      // This ensures background music and sangeet music are stopped
      ...
      // Play mangalam music - only if audio is loaded
          ...
          playPromise
            ...
            .catch((e) => {
              ...
              if (e.name !== 'NotAllowedError') {
                console.error('Failed to play mangalam.mp3:', e);
              }
              isPlayingRef.current = false;
            });
        } else {
      ...
      // Leaving wedding section - stop mangalam music immediately and aggressively
      ...
      if (audio) {
        ...
        try {
          ...
          // The monitoring loop will handle volume when we re-enter wedding section
          isPlayingRef.current = false;
        } catch (e) {
  ...
  // Continuous monitoring effect - ensures audio stops if we're not in wedding section
  ...
  useEffect(() => {
    ...
    const checkInterval = setInterval(() => {
      if (audioRef.current) {
        ...
        // Recalculate if we're in wedding section based on current progress
        const currentlyInSection = currentProgress >= WEDDING_START && currentProgress <= WEDDING_END;
        ...
        if (!currentlyInSection) {
          ...
          if (audio) {
            ...
            audio.pause();
            audio.currentTime = 0;
            audio.volume = 0; // Keep volume at 0 when not in section - NEVER restore until we're back in wedding section
            isPlayingRef.current = false;
        ...
        // If we're in section but audio is not playing, start it
          ...
          // First, stop all other audio before starting wedding music
          const allAudioElements = document.querySelectorAll('audio');
          allAudioElements.forEach((audioEl) => {
            if (audioEl !== audioRef.current) {
              audioEl.volume = 0;
...
export default WeddingMusicManager;
